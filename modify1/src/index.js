...
var formatHtml = require('./lib/formatters/html');
...
function lintFiles(options, reporter) {
	...
	function bufferContents(file, enc, cb) {
		...
		lint.then(function(issues) {
			...
			for(var i=0; i<options.filename.length; i++) {
				if (file.path.indexOf(options.filename[i]) > -1) {
					widgetName = options.filename[i];
				}
			}
			var result = {
				filepath: file.path,
			...
			if (typeof reporter === 'function') {
				//reporter(file.path, issues, options.reporter);
			} else {
				if (issues.length > 0) {
					out.push('\n' + file.path);
				}
	...
	function endStream(cb) {
		...
		if (typeof reporter === 'function') {
			reporter(results, options.reporter, options.filename);
		}
		this.emit('end');
		cb();
	}
...
lintFiles.formatHtml = formatHtml;
